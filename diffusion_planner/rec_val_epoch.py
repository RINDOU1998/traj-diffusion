...
def validation_epoch(model, val_loader, device,show = False ):
    ...
    for batch in tqdm(val_loader, desc="Validation", unit="batch"):
        ...
        score = decoder_outputs["score"][:,  :19, :] # [B, P, T, 2] 
        gt = decoder_outputs["gt"][:, :19, :] # [B, P, T, 2]

        recon_loss = (score - gt)**2
    ...
    print(f"✅ Validation - Loss: {avg_loss:.4f}, ADE: {ade:.4f}")
    return ade
